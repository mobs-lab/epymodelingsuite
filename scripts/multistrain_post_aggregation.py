...
def main():
    ...
    try:
        ...
        if config.surveillance.recent_fname is not None:
            ...
            surv_recent = pd.read_csv(
                f"{config.surveillance.directory}/{config.surveillance.recent_fname}"
            )
    except Exception as e:
    ...
    surv_fit = surv_fit[[config.surveillance.date_column,
                         "abbreviation",
                         config.surveillance.target_column]]
    if surv_recent is not None:
        ...
        surv_recent = surv_recent[[config.surveillance.date_column,
                                   "abbreviation",
                                   config.surveillance.target_column]]
    ...
    # Format trajectories
    aggregated = aggregated[[config.aggregated.date_column,
    ...
    if config.single_strain is None:
        ...
        # Format trajectories
        single_strain = single_strain[[config.single_strain.target_column,
    ...
    print(
        ...
        (pmf_df[RATE_TREND_CATEGORIES].sum(axis=1).round(6) == 1).all()
        }"
    )
    ...
    for plot_focus, plotting_window in plotting_windows.items():
        ...
        # Format data
        agg_quantiles_filter = agg_quantiles[
            ...
            columns={config.aggregated.date_column: "date"}
        )
        single_quantiles_filter = single_quantiles[
            ...
            columns={config.single_strain.date_column: "date"}
        )
        surv_fit_filter = surv_fit[
            ...
            config.surveillance.date_column
        ).rename(
            columns={config.surveillance.date_column: "date",
                     config.surveillance.target_column: "target"}
        )
        surv_recent_filter = None
        if surv_recent is not None:
            surv_fit_filter = surv_fit_filter[
                surv_fit_filter["date"] < reference_dt
            ]
        ...
        plot_hosp_multistrain_quantiles(
            ...
            fit_start_labels = {s:l for s,l in fit_start_labels.items() if plot_start_date<=pd.Timestamp(s)<=plot_end_date},
