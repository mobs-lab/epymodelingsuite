...
def make_school_closure_dict(
	years: list[int],
	interval_reduction_iter: Optional[int] = 5
	...
	for year in years:
		...
		thanksgiving = [item[0] for item in country_holidays('US', years=year).items() if item[1] == 'Thanksgiving Day'][0].day
	...
	closure_df = closure_df.assign(start=closure_df.start_date.map(date_to_int),
								   end=closure_df.end_date.map(date_to_int),
								   name='US Combined School Closure') # setting names equal (we want to combine without grouping)
	...
	closure_reduced = closure_reduced.assign(start_date=closure_reduced.start.map(int_to_date),
											 end_date=closure_reduced.end.map(int_to_date), 
											 name=closure_reduced.name)[['start_date', 'end_date', 'name']]
	
	# Make a set of Closures from the reduced df, this will be added to the closure dict
	us_closures = set(Closure(closure.start_date, closure.end_date, closure.name) for closure in closure_reduced.itertuples(index=False))
	...
	def date_in_range(start, end, comp): return start <= comp <= end
	...
	for loc in closure_dict:
		...
		[closure_dict[loc].add(Closure(holiday[0], holiday[0], holiday[1]))
		 ...
		 if not any([date_in_range(closure.start_date, closure.end_date, holiday[0]) 
...
def add_school_closure_interventions(
	...
	[model.add_intervention(layer_name='school', start_date=closure.start_date, end_date=closure.end_date,
							reduction_factor=reduction_factor, name=closure.name) for closure in closures]
