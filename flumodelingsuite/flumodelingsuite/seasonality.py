...
def calc_seasonality_balcan_at_date(
    ...
    R_min: float,
    R_max: float,
    date_tmin: Optional[dt.date] = None,
    period: Optional[int] = None
) -> float:
    """
    Computes the seasonal rate for a given date using the Balcan model.
...
def generate_seasonal_values(
    date_start: dt.date,
    date_stop: dt.date,
    seasonality_func: Callable[[dt.date], float]
) -> tuple[list[dt.date], list[float]]:
    """
    A generic function to generate values over a date range using any seasonality function.
...
def get_seasonal_transmission_balcan(
    ...
    '''
    Returns seasonal transmission rates for the specified simulation period using the Balcan model.
